{ config, pkgs, lib, ... }:
{
  programs.git = {
    enable = true;

    lfs.enable = true;

    signing = {
      key = "ssh-rsa ";
      format = "ssh";
      signByDefault = true;
    };

    ignores = [
      ".DS_Store"
      ".AppleDouble"
      ".LSOverride"
      "Icon\r\r"
      "._*"
      ".DocumentRevisions-V100"
      ".fseventsd"
      ".Spotlight-V100"
      ".TemporaryItems"
      ".Trashes"
      ".VolumeIcon.icns"
      ".com.apple.timemachine.donotpresent"
      ".AppleDB"
      ".AppleDesktop"
      "Network Trash Folder"
      "Temporary Items"
      ".apdisk"
      "logs"
      "*.log"
      "npm-debug.log*"
      "yarn-debug.log*"
      "yarn-error.log*"
      ".vscode"
      ".zed"
    ];

    settings = {
      user = {
        email = "10683430+ETeissonniere@users.noreply.github.com";
        name = "Eliott Teissonniere";
      };
      core.autocrlf = "input";
      pull.rebase = true;
      submodule.recurse = true;
      push.autoSetupRemote = true;
      init.defaultBranch = "master";
      gpg.ssh.allowedSignersFile = "~/.config/git/allowed_signers";
    };
  };

  xdg.configFile."git/allowed_signers".source = ../../../config/git/allowed_signers;
}
